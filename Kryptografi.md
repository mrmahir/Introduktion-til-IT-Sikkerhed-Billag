...
Person A generates a public and private key
```console
...
```
Person B uses person A's public key to encrypt their message
```console
...
```
Person A takes person B's encrypted message and decrypts it using their own corresponding private key
```console
